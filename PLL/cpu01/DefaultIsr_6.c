...
int16 ADC_entero;
float32 ADC_flotante;
float32 AdcBufFlot_norm[100], AdcBufFlot_sal[100], AdcBufFlot_phase[100];
Uint16 AdcBufint[100];
float32 dato_proc_flot;
Uint16 dato_proc_entss;
float32 yk = 0;                 // Variables de retroalimentación
float32 xk = 0;                 // Variables de entrada
float32 epd = 0, epd_1 = 0;     // Variables de error
float32 vif = 0, vif_1 = 0;     // Variables de salida del PI
float32 w = 0, w_1 = 0;         // Variables de frecuencia W
float32 phase = 0, phase_1 = 0; // Variables de fase
Uint16 select_output = 0;       // Variable de selección de salida
float32 pll_out;                // Variable de salida del PLL

//---------------------------------------------------------------------
interrupt void ADCA1_ISR(void)						// PIE1.1 @ 0x000D40  ADC-A interrupt #1
{
static float32 *AdcBufFlot_normPtr = AdcBufFlot_norm;         // Puntero del buffer de la senal normalizada
static float32 *AdcBufFlot_salPtr = AdcBufFlot_sal;           // Puntero del buffer de la senal de salida del PLL
static float32 *AdcBufFlot_phasePtr = AdcBufFlot_phase;       // Puntero del buffer de la fase del PLL
static Uint16 *AdcBufIntPtr = AdcBufint;
static volatile Uint16 GPIO34_count = 0;			    // Counter for pin toggle

    // Pone a 1 el bit
    GpioDataRegs.GPASET.bit.GPIO0 = 1;                  // Pin GPIO0 a 1
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP1;			    // Must acknowledge the PIE group
	//--- Manage the ADC registers
	AdcaRegs.ADCINTFLGCLR.bit.ADCINT1 = 1;			    // Clear ADCINT1 flag
	//--- Read the ADC result
	ADC_entero = (AdcaResultRegs.ADCRESULT0) - 2047;    //tipo int16 resta 2047 de offset
	ADC_flotante = (float32)ADC_entero;                 // conversion de entero a punto flotante
	ADC_flotante = 0.0006447453255963894*ADC_flotante;  // conversion de señal de +-1551 a +-1
	*AdcBufFlot_normPtr++ = ADC_flotante;               // Almacenamiento en el buffer

	xk = ADC_flotante;                                  // Asignación de la variable de entrada del filtro
	epd = xk*yk;                                        // Cálculo del error
	vif = A1*vif_1 + B0*epd + B1*epd_1;                 // Calculo del filtro de lazo
	epd_1 = epd;
	vif_1 = vif;
	w = vif + 2*3.14159265358979323846*60;              // Calculo de W con respecto a una frecuencia angular = 2*pi*60
	phase = C1*phase_1 + D0*w + D1*w_1;                 // Calculo del integrador
	if(phase > 2*3.14159265358979323846)                // Reset del integrador
	    phase = 0;
	w_1 = w;
	phase_1 = phase;
	yk = cosf(phase);                                   // Calculo de la retroalimentacion
	pll_out = sinf(phase);                              // Calculo de la salida

	*AdcBufFlot_salPtr++ = pll_out;                     // Almacenamiento en buffer
	*AdcBufFlot_phasePtr++ = phase;                     // Almacenamiento en buffer

//--- Brute-force the circular buffer
	if( AdcBufFlot_normPtr == (AdcBufFlot_norm + ADC_BUF_LEN) )
	{	
	    AdcBufFlot_normPtr = AdcBufFlot_norm;					// Rewind the pointer to beginning
	}

	if( AdcBufFlot_salPtr == (AdcBufFlot_sal + ADC_BUF_LEN) )
    {
	    AdcBufFlot_salPtr = AdcBufFlot_sal;                     // Rewind the pointer to beginning
    }

    if( AdcBufFlot_phasePtr == (AdcBufFlot_phase + ADC_BUF_LEN) )
    {
        AdcBufFlot_phasePtr = AdcBufFlot_phase;                 // Rewind the pointer to beginning
    }
...
//             ==> x = 25000
	...
	if(select_output == 0)                          // Seleccion de la salida por la DAC, 0: salida del PLL, 1: fase
        dato_proc_flot = 1240.9*(0.5*pll_out + 1.6);    // Escala nuevamente de +-1 a +-0.5 y offset +1.5
                                                        // y luego escalamos a 0-4095 para la DAC
	else
	    dato_proc_flot = 1240.9*0.5*phase;          // Escalado de 0-2PI a 0-PI y luego escalamos a 0-4095 para la DAC
    dato_proc_entss= (Uint16)dato_proc_flot;        // Convierte de float a int s/signo
    *AdcBufIntPtr++ = dato_proc_entss;
    DacbRegs.DACVALS.all = dato_proc_entss;         // Dato de salida por la DAC

    if( AdcBufIntPtr == (AdcBufint + ADC_BUF_LEN) )
    {
        AdcBufIntPtr = AdcBufint;                 // Rewind the pointer to beginning
    }

    // Pone a 0 el bit
    GpioDataRegs.GPACLEAR.bit.GPIO0 = 1;              // Pin GPIO0 a 0
}
