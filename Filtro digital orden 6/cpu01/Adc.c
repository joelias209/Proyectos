...
//--- SOC0 configuration
	AdcaRegs.ADCSOC0CTL.bit.TRIGSEL = 7;		// Trigger (ePWM2SOCA/C)
	AdcaRegs.ADCSOC0CTL.bit.CHSEL = 0;			// Convert channel (ADCIN0)
	AdcaRegs.ADCSOC0CTL.bit.ACQPS = 19;			// Acquisition window (20*SYSCLK cycles wide)

	AdcaRegs.ADCINTSOCSEL1.bit.SOC0 = 0;		// ADC interrupt triggers (no ADC will trigger SOCx <TRIGSEL field determinates SOCx trigger)

	AdcaRegs.ADCSOCPRICTL.bit.SOCPRIORITY = 0;	// SOC priority mode (round robin mode in all channels
