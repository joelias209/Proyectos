...
float32 AdcBufFlot[220];
float32 dato_proc_flot;
Uint16 dato_proc_entss;
float32 yk = 0, yk_1 = 0, yk_2 = 0, yk_3 = 0, yk_4 = 0, yk_5 = 0, yk_6 = 0;     // Variables de salida actual y pasadas
float32 xk = 0, xk_1 = 0, xk_2 = 0, xk_3 = 0, xk_4 = 0, xk_5 = 0, xk_6 = 0;     // Variables de entrada actual y pasadas

//---------------------------------------------------------------------
interrupt void ADCA1_ISR(void)						// PIE1.1 @ 0x000D40  ADC-A interrupt #1
{
static int16 *AdcBufPtr = AdcBuf;					// Pointer to buffer
static float32 *AdcBufFlotPtr = AdcBufFlot;           // Apuntador a la primera localidad del buffer
static volatile Uint16 GPIO34_count = 0;			// Counter for pin toggle
	...
	*AdcBufPtr++ = 	AdcaResultRegs.ADCRESULT0;			// Read the result
	ADC_entero = (AdcaResultRegs.ADCRESULT0) - 2070;    //tipo int16 resta 2047 de offset
	ADC_flotante = (float32)ADC_entero;                 // conversion de entero a punto flotante
	ADC_flotante = 0.0025789813023856*ADC_flotante;     // conversion de señal de +-1861 a +-4
	*AdcBufFlotPtr++ = ADC_flotante;                    // Almacenamiento en el buffer

	xk = ADC_flotante;                                  // Asignación de la variable de entrada del filtro
    yk = A1*yk_1 + A2*yk_2 + A3*yk_3 + A4*yk_4 + A5*yk_5 + A6*yk_6 +            // Ecuación de diferencias
         B0*xk + B1*xk_1 + B2*xk_2 + B3*xk_3 + B4*xk_4 + B5*xk_5 + B6*xk_6;
    yk_6 = yk_5;                                        // Asignación de las variables de entrada y
    yk_5 = yk_4;                                        // salidas anteriores
    yk_4 = yk_3;
    yk_3 = yk_2;
    yk_2 = yk_1;
    yk_1 = yk;
...
//--- Brute-force the circular buffer
	...
	if( AdcBufFlotPtr == (AdcBufFlot + ADC_BUF_LEN) )
    {
        AdcBufFlotPtr = AdcBufFlot;                     // Rewind the pointer to beginning
    }
...
//             ==> x = 25000
    ...
    dato_proc_flot = 1240.9*(0.25*yk + 1.5);        // Escala nuevamente de +-4 a +-1 y offset +1.5
