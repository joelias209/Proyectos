...
void test_splay(int n, int caso){
    ...
    if(caso == ALEATORIO){
        for(int i=0; i<n; i++){
            if(find_splay(&root, datos[i]) == NO)
                printf("Elemento %d no encontrado!\n", datos[i]);
        }
    }
    else if(caso == CRECIENTE){
        for(int i=0; i<n; i++){
            if(find_splay(&root, i) == NO)
                printf("Elemento %d no encontrado!\n", i);
        }
    }
    else{
        for(int i=0; i<n; i++){
            if(find_splay(&root, n-i-1) == NO)
                printf("Elemento %d no encontrado!\n", n-i-1);
        }
    }
    gettimeofday(&end, NULL);
