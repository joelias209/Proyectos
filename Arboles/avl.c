...
void test_avl(int n, int caso){
    ...
    if(caso == ALEATORIO){
        for(int i=0; i<n; i++){
            if(find_avl(root, datos[i]) == NO)
                printf("Elemento %d no encontrado!\n", datos[i]);
        }
    }
    else if(caso == CRECIENTE){
        for(int i=0; i<n; i++){
            if(find_avl(root, i) == NO)
                printf("Elemento %d no encontrado!\n", i);
        }
    }
    else{
        for(int i=0; i<n; i++){
            if(find_avl(root, n-i-1) == NO)
                printf("Elemento %d no encontrado!\n", n-i-1);
        }
    }
    gettimeofday(&end, NULL);
...
int main(int argc, const char * argv[]){
    ...
    for(int i=100; i<n1; i+=100)
        test_avl(n1, ALEATORIO);

    for(int i=n1; i<n2; i+=1000)
        test_avl(i, ALEATORIO);
